...
import { Search, Calendar, MapPin, Users, Heart, Share2, Info, ChevronRight, Globe, Bookmark, Loader2 } from 'lucide-react';
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <main className="max-w-6xl mx-auto p-12">
           ...
           <div className="flex gap-3 mb-12 overflow-x-auto pb-4 scrollbar-hide">
              {['近日開催のイベント', 'ワークショップ', '言語交換', '交流会'].map((cat, i) => (
                <button 
                   key={cat} 
           ...
           <div className="flex flex-col lg:flex-row gap-12">
              ...
              <div className="flex-1 space-y-8">
                 <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{events.length}件のイベントが見つかりました</span>
                 </div>

                 {loading && <div className="p-8 text-center"><Loader2 className="w-8 h-8 text-[#0F4186] animate-spin mx-auto" /></div>}
                 {!loading && events.length === 0 && <div className="p-8 text-center text-slate-500">イベントがありません。</div>}
