...
import { Star, Send, X, ShieldCheck, MessageSquareText, Loader2, Languages } from 'lucide-react';
...
export default function ReviewView({ buddyId, onBack, onSuccess, isTranslateOn, onToggleTranslate }: ReviewViewProps) {
  ...
  const handleSubmit = async () => {
    if (rating === 0) {
      setError(t('評価（星）を選択してください。', 'Vui lòng chọn số sao đánh giá.'));
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(t('レビューの送信に失敗しました。', 'Gửi đánh giá thất bại.'));
      }
      onSuccess();
    } catch (err: any) {
    ...
    <div className="fixed inset-0 z-[100] bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4">
      ...
      >
        <div className="h-40 shrink-0 w-full bg-gradient-to-br from-[#0F4186] to-[#0D3875] relative flex items-center justify-center">
            <div className="absolute top-8 right-20 select-none">
              <div className="flex items-center gap-2 bg-white/10 backdrop-blur-md px-3 py-1 rounded-full border border-white/20 shadow-sm text-white">
                 <Languages className="w-3.5 h-3.5 text-blue-200" />
                 <span className="text-[8px] font-black text-blue-100 uppercase tracking-widest">{t('自動翻訳', 'Tự động dịch')}</span>
                 <button 
                    type="button"
                    onClick={onToggleTranslate}
                    className={`w-9 h-4.5 rounded-full relative transition-all ${isTranslateOn ? 'bg-blue-500' : 'bg-white/20'}`}
                 >
                    <div className={`absolute top-0.5 w-3.5 h-3.5 bg-white rounded-full transition-all ${isTranslateOn ? 'right-0.5' : 'left-0.5'}`} />
                 </button>
                 <p className="text-[8px] font-black text-blue-200 border-l border-white/20 pl-2 leading-none">JP ↔ VN</p>
              </div>
            </div>
            ...
            <div className="absolute top-10 left-12">
               <h2 className="text-white text-2xl font-black italic opacity-20 tracking-widest uppercase select-none">{t('評価', 'Feedback')}</h2>
            </div>
        </div>

        <div className="p-8 lg:p-12 -mt-10 bg-white rounded-t-[40px] relative z-10 flex-1 overflow-y-auto custom-scrollbar">
          <div className="text-center mb-10">
            <h3 className="text-2xl font-bold text-slate-900 mb-2">{t('インタラクションを評価する', 'Đánh giá tương tác')}</h3>
            <p className="text-slate-500 text-sm">
              {t('体験を評価してください', 'Hãy đánh giá trải nghiệm của bạn')}
            </p>
          </div>
          ...
          <div className="text-center mb-10 h-8">
             <p className={`text-xl font-bold transition-all duration-500 ${rating > 0 ? 'text-rose-500 opacity-100' : 'text-slate-300 opacity-0'}`}>
                {rating === 5 ? t('素晴らしい！', 'Rất tuyệt!') : 
                 rating === 4 ? t('とても良い', 'Rất tốt') : 
                 rating === 3 ? t('普通', 'Bình thường') : 
                 rating === 2 ? t('あまり良くない', 'Không tốt lắm') : 
                 rating === 1 ? t('非常に悪い', 'Rất tệ') : ''}
             </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-8">
            {[
              { id: 'rel', label: t('信頼性', 'Độ tin cậy'), icon: ShieldCheck },
              { id: 'comm', label: t('コミュニケーション', 'Giao tiếp'), icon: MessageSquareText },
            ].map((cat) => (
              <button
                key={cat.id}
          ...
          <div className="mb-10">
            <div className="flex justify-between items-center mb-3 px-1">
               <label className="text-xs font-bold text-slate-800 uppercase tracking-widest">{t('レビューを記入', 'Viết nhận xét')}</label>
               <span className="text-[10px] font-bold text-slate-400 uppercase">{text.length}/500{t('文字', ' kí tự')}</span>
            </div>
            <textarea 
              value={text}
              ...
              placeholder={t('セッションはどうでしたか？何を学びましたか？（日本語またはベトナム語で入力してください）', 'Buổi học thế nào? Bạn đã học được gì? (Nhập bằng tiếng Nhật hoặc tiếng Việt)')}
            />
          </div>
          ...
          <div className="space-y-4">
             ...
             >
                {isSubmitting ? (
                  ...
                  <>
                    <span>{t('評価を送信する', 'Gửi đánh giá')}</span>
